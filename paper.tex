...
\begin{abstract}
We perform follow-up N-body simulations on some of the most massive galaxies extracted from a cosmological simulation of hierarchical structure formation with total masses in the range $10^{12} M_{\sun} < M < 10^{14} M_{\sun}$ from 4 $\gid$ z $\gid$ 0.  After galactic mergers, we track the dynamical evolution of the infalling black holes (BHs) around their host's central BHs.  From 11 different simulations, we find that, of the 86 infalling BHs with masses > XYZ, 36 merge with their host's central BH, 13 are ejected from their host galaxy, and 37 are still orbiting at the end of the simulations.  Across all galaxies, 33 BHs are kicked to a higher orbit after close interactions with the central BH binary or multiple, after which only one of them merged with their hosts.  These orbiting BHs should be detectable by their anomalous (not LMXRB) spectra.  The x-ray luminosities of the orbiting massive BHs at z=0 are in the range $10^{25}-10^{43}$ $erg\ s^{-1}$, with an undetectable median value $10^{32}$ $erg\ s^{-1}$.  However, the most luminous $\sim$$5\%$ should be detectable by existing x-ray facilities.
\end{abstract}
...
The merger tree data that was used as the input to our simulations is the result of previous work by \citet{2011ApJ...741...99C, 2011ApJ...742L..33C, 2012ApJ...753...17C, 2012ApJ...748..121C, 2013ApJ...770..139C} (hereafter Cen(+11) and known as the LAOZI simulations \citet{2014ApJ...781...38C}), as well as  \citet{2012MNRAS.425..641L} (hereafter Lackner(+12)), and \citet{2015ApJ...799..178K} (hereafter Kulier(+15)).  A brief synopsis of the work that led to our input data is described below.
...
\begin{equation}
    \dot{M}_{acc} = f\frac{4{\pi}G^2M^2\rho}{(C_s^2+v^2)^{3/2}} \text{  M}_\odot \text{ Myr}^{-1},
\end{equation}
...
\end{equation}
where the x-ray radiative efficiency ${\epsilon}_{rx}=0.01$, and we are assuming for the low-luminosity sources that $L_B{\sim}L_x$.  For sources having $\dot{M}_B/\dot{M}_{Edd} > 10^{-3}$, we take $L_x=0.1L_{Bol}$.
