...
\begin{abstract}
We perform N-body simulations on 13 galaxies extracted from a cosmological simulation of hierarchical structure formation with total mass in the range $10^{12} \msun < M < 10^{13} \msun$ from $4 \ge z \ge 0$.  We track the dynamical evolution of the orbiting black holes (BHs) around their host central BH.  Of the 120 orbiting BHs, we find that 50 merge with their host central BHs, 11 are ejected from their respective host galaxies, 59 are still orbiting at the end of the simulations.  Across all galaxies, 48 BHs are kicked to a higher orbit after close interaction with the central BH, after which none of them merge with their hosts.  We calculate the x-ray luminosities of the orbiting massive BHs at z=0, and find a range of $10^{18}-10^{37}$ $erg\ s^{-1}$.
\end{abstract}
...
Figure \ref{fig:meosmbh} shows the distribution of merged, ejected and orbiting black holes across all galaxy simulations as a function of guest black hole mass.  Given the log distribution of masses and using $10^7\, M_{\odot}$ as the approximate midpoint, one can see the general trend of larger mass BHs tending to merge with their respective host BH, with     the lower mass BHs continuing to wander at $z=0$.
...
\caption{Histogram of kicked SMBHs at z=0.}
\label{fig:kicked_stats}
\end{center}
\end{figure}

As can be seen in Figure \ref{fig:kicked_stats}, the vast majority of larger SMBHs ($\gtrapprox\, 10^7\, M_{\odot}$) are not kicked to higher orbits nor ejected from their galaxies, which at least partly explains why most of the merged SMBHs are in the greater mass brackets.  In contrast, most of the SMBHs below $10^7\, M_{\odot}$ are kicked (with a few of them being ejected).  in our simulations, no black holes that are kicked to higher orbits ever merged with their central host SMBH.
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=1.0\columnwidth]{plots/"radiative efficiency".png}
